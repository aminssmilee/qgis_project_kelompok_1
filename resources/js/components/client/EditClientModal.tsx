...
export default function EditClientModal({ client, onClose, onSuccess }: EditClientModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast.success(`Data klien ${updatedClient.name} berhasil diperbarui!`);
            onSuccess(updatedClient);
        } catch (err: any) {
            console.error(err);
            toast.error(err.response?.data?.message || "Gagal memperbarui klien");
        } finally {
            ...
            <div className="fixed inset-0 flex items-center justify-center z-[9999] p-4 pointer-events-none">
                <Card className="w-full max-w-md shadow-2xl pointer-events-auto">
                    <CardHeader className="flex flex-row items-center justify-between border-b">
                        <CardTitle className="text-lg">Edit Data Klien</CardTitle>
                        <button
                            onClick={onClose}
