...
export default function EditUserModal({ user, onClose, onSuccess }: EditUserModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(err);
            toast.error(err.response?.data?.message || "Gagal memperbarui data user");
        } finally {
    ...
    return (
        <FormModal title="Edit User" onClose={onClose}>
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-2">Password Baru <span className="text-gray-400 font-normal">(Kosongkan jika tidak ingin mengubah)</span></label>
                    <input
                        type="password"
                ...
                <div className="flex gap-3 pt-4">
                    ...
                    <Button type="submit" className="flex-1 gap-2" disabled={isSubmitting}>
                        {isSubmitting ? "Menyimpan..." : <><Save className="h-4 w-4" /> Simpan Perubahan</>}
                    </Button>
                </div>
            </form>
        </FormModal>
